...
The local daemon polls `gh issue list` and `gh pr list`, creates local events under `.ai/local-daemon/events/`, records processed triggers in `.ai/local-daemon/state.json`, and can wake the full local automation chain from label or comment triggers. Trigger mappings live in `.ai/rules/local-daemon.yml`, so teams can choose their own labels and slash commands without code changes. Poll summaries are written to `.ai/local-daemon/polls/`. Each created event writes `event_result.json`; with `--status-sync`, the daemon also posts a short redacted planned or final status comment back to the source issue or PR. Runtime logs, traces, prompts, and connector output remain local and git-ignored.
